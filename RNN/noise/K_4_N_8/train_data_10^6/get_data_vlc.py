...
# -*- coding: utf-8 -*-
"""
Created on Thu May 10 13:48:25 2018
VLC channel model 
@author: dung-le
"""
...
'''
VLC channel parameter
'''
q = 1.6e-19         # Charge of Electron
Ib = 202e-6         # Background Noise Current + Interence
N0 = 2*q*Ib         # Noise Spectral Density, 2*q*Ib
R = 1               # Photodetector responsivity
Rb = 1e6            # Bit rate
Tb = 1/Rb           # bit duration
nsamp = 1           # samples per symbols
...
def ook(x, SNR):
    S = 10**(SNR/10)
    P_avg = np.sqrt((N0*Rb*S)/(2*R**2))
    i_peak = 2*R*P_avg
    Ep = i_peak**2*Tb
    return x*i_peak

def add_noise(x, SNR):
    sigma = np.sqrt(N0/2/Tsamp)
    if len(x.shape) == 1:
...
for ratio in train_ratio:
    ...
    for snr in train_snr:
        ...
        for i in range(num_train):
            ...
            x_train[i] = add_noise(ook(x_train_selected[random_index2],snr), snr)
...
for snr in test_snr:
    ...
    for i in range(num_test):
        ...
        x_test[i] = add_noise(ook(x_data[random_index3], snr), snr)
